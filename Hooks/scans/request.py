...
class request(loadable):
    ...
    def list(self, message, user, params):
        ...
        message.reply(" ".join(map(lambda (request, count): Config.get("Scans", "reqlist").decode("string_escape") % (request.id, request.target.intel.dists if request.target.intel else "0",
    ...
    def links(self, message, user, params):
        ...
        message.reply(self.url(" ".join(map(lambda (request, count): Config.get("Scans", "reqlinks").decode("string_escape") % (request.id, 
