...
class request(loadable):
            ...
            for scantype in params.group(6).upper():
                ...
                if galscan or mergescans:
                    ...
                    if message.get_chan() != self.scanchan():
                        message.reply("Requested a %s Scan of %s:%s:%s%s. !request cancel %s to cancel the request." % (request.type, planet.x, planet.y, planet.z, (" (%s)" % planet.state) if planet.state else "", request.id,))
    ...
    def robocop(self, message, request_id, mode):
        ...
        message.privmsg("[%s] %s requested a %s Scan of %s:%s:%s%s Dists(i:%s%s) " % (request.id, requester, request.type, planet.x,planet.y,planet.z, (" (%s)" % planet.state) if planet.state else "",
    ...
    def cancel(self, message, user, params):
        ...
        for id in cancel_ids:
            ...
            if request is None:
                noexist.append(id)
                continue
            if request.user is not user and not self.check_access(message, "req_cancel"):
                noaccess.append(id)
                continue
    ...
    def blocks(self, message, user, params):
        ...
        if request is None:
            ...
            return
        if request.user is not user and not self.check_access(message, "req_update"):
    ...
    def list(self, message, user, params):
        ...
        message.reply(" ".join(map(lambda request_count: Config.get("Scans", "reqlist").decode("string_escape") % (request_count[0].id, request_count[0].target.intel.dists if request_count[0].target.intel else "0",
    ...
    def links(self, message, user, params):
        ...
        message.reply(self.url(" ".join(map(lambda request_count: Config.get("Scans", "reqlinks").decode("string_escape") % (request_count[0].id, 
